...
public interface PhoneLoginService {
     ...
     * @return MessageAuthCodeLoginDTO 该对象表示这次发送验证码的信息
     */
    Result<MessageAuthCodeLoginDTO> createMessageAuthCodeAndSend(
     ...
     */
    Result<Void> checkMessageAuthCode(
            @NotNull(message = "INVALID_PARAMETER_IS_NULL: The phone must be not null.")
            @Phone String phone,
            @NotNull(message = "INVALID_PARAMETER_IS_NULL: The messageAuthCode must be not null.")
            @MessageAuthCode String authCode);
