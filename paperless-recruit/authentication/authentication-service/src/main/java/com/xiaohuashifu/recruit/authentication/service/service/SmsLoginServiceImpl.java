...
public class SmsLoginServiceImpl implements SmsLoginService {
    ...
    public SmsLoginServiceImpl(RedisTemplate<Object, Object> redisTemplate) {
        this.redisTemplate = redisTemplate;
    }

    /**
     * 短信验证码登录的创建并发送手机短信验证码服务
    ...
    public Result<SmsLoginDTO> createAndSendSmsAuthCode(String phone) {
        ...
        // 添加短信验证码到缓存
        String redisKey = SmsLoginServiceConstant.SMS_AUTH_CODE_REDIS_PREFIX + phone;
        redisTemplate.opsForValue().set(redisKey, smsAuthCode, smsAuthCodeExpiredTime, TimeUnit.SECONDS);

        return Result.success(new SmsLoginDTO.Builder()
                .phone(phone)
                .authCode(smsAuthCode)
                .message(message)
                .expireTime(LocalDateTime.now().plusSeconds(smsAuthCodeExpiredTime))
                .build());
    }
     ...
     */
    @Override
    public Result<Void> checkSmsAuthCode(String phone, String authCode) {
        // 从缓存取出验证码
        String redisKey = SmsLoginServiceConstant.SMS_AUTH_CODE_REDIS_PREFIX + phone;
        ...
        // 验证码不正确
        if (!smsAuthCode.equals(authCode)) {
